var http = require('http'), fs = require('fs'), path = require('path'), CONFIG = require('../lib/config'), moment = require('moment');
...
http.createServer(function (req, res) {
  ...
  else if(req.url.includes('/?action=snapshot')){
    var filename = '/var/www/openrov/photos/';
    var time = req.url.split('-').pop();
    var ext = '.jpeg';
    fs.createReadStream(file).pipe(fs.createWriteStream(filename + time + ext));
  } 
...
function grab() {
  ...
  file = path.join(imagePath, mockImages[nextImageIndex]);
